...
#define DEVICE_NAME "pid2task"         //  定义设备文件名    
...
static ssize_t pid_to_task_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)    
{       
...
static ssize_t pid_to_task_write(struct file *file, const char __user *buf, size_t count, loff_t *ppos)    
{       
...
{ .owner = THIS_MODULE, .read = pid_to_task_read, .write = pid_to_task_write };    
...
//  初始化Linux驱动    
static int pid_to_task_init(void)    
{    
    int ret;    
    //  建立设备文件    
    ret = misc_register(&misc);    
    //  输出日志信息    
    printk("pid_to_task_init_success\n");    
    return ret;    
}    
     
// 卸载Linux驱动    
static void pid_to_task_exit(void)    
{    
    //  删除设备文件    
    misc_deregister(&misc);    
    //  输出日志信息    
    printk("pid_to_task_exit_success\n");    
}    
     
//  注册初始化Linux驱动的函数    
module_init( pid_to_task_init);    
//  注册卸载Linux驱动的函数    
module_exit( pid_to_task_exit);    
     
MODULE_AUTHOR("elwin_mao");    
MODULE_DESCRIPTION("get process info.");    
MODULE_ALIAS("pid_to_task module.");    
MODULE_LICENSE("GPL");
